<x-app-layout>
    ...
    <div class="md:container md:mx-auto bg-indigo-50 rounded-3xl py-14 border px-10 box-border shadow mb-16 relative "> 
        <div class="flex justify-between mb-10 items-center">
            ...
            <a href=" {{route('role.create')}} " class="bg-indigo-600 px-3 py-2 rounded-xl text-white">Create Role</a>
        </div>
            ...
            <tbody>
                @foreach ($roles as $role)       
                    <tr class="border-b-1 border-gray-300 text-[15px] ">  
                        ...
                        <td  class="py-4 w-[8%]"><a href="{{ route('role.give-permissions', $role->id) }}" class="pl-6 pr-0 text-indigo-600 font-medium">Permission</a></td>
                        <td  class="py-4 w-[1%]"><a href="{{ route('role.edit', $role->id) }}" class="pl-6 pr-0 text-indigo-600 font-medium">Edit</a></td>
                        <td class="py-4">
                            <form action="{{ route('role.delete', $role->id) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                                @csrf
                                @method('DELETE')
